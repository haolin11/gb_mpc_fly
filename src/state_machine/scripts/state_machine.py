...
class Algorithm:
    ...
    def is_close(self, odom: Odometry, pose: list):
        ...
        if self.state == 1:
            if abs(odom.pose.pose.position.x - pose[0]) < 0.2 and abs(odom.pose.pose.position.y - pose[1]) < 0.2 and abs(odom.pose.pose.position.z - pose[2]) < 0.1:
                return True
        else:
            if abs(odom.pose.pose.position.x - pose[0]) < 0.2 and abs(odom.pose.pose.position.y - pose[1]) < 0.2:
                return True
