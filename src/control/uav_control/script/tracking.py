...
setpositionn_pub = rospy.Publisher(
...
def odom_cb(msg: Odometry):
    ...
    target_pose.pose.position.z = msg.pose.pose.position.z
    ...
    if trajectory != None and len(trajectory._poss) != 0:
        ...
        # u.thrust = 0
        target_pose.pose.position.x = poss[4][0]
        target_pose.pose.position.y = poss[4][1]
        target_pose.pose.position.z = poss[4][2]
    if state_machine.data < 9:
        setpoint_raw_pub.publish(u)
        print(u.thrust, u.body_rate.x, u.body_rate.y, u.body_rate.z)

        # setpositionn_pub.publish(target_pose)
